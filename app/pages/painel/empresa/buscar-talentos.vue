...
const openWhatsApp = async (candidate: any) => {
    if (!candidate?.telefone || !user.value) return
    
    try {
        // IDs ordenados para manter a unicidade (p1 sempre menor que p2)
        const [p1, p2] = [user.value.id, candidate.id].sort()
        ...
        if (existing) {
            // Se já existe, garante que o tipo_contato seja refletido
            await supabase
                .from('conversas')
                .update({ 
            ...
            await supabase
                ...
                .insert({
                    ...
                    ultima_mensagem: 'Iniciado via WhatsApp'
                })
        }

        // Abre o WhatsApp com mensagem padronizada
        const cleanPhone = candidate.telefone.replace(/\D/g, '')
        const message = encodeURIComponent(`Olá ${candidate.nome}, vi seu perfil no PebasPro e gostaria de conversar sobre uma oportunidade!`)
                    ...
                    <div class="flex-1">
                        <div class="flex flex-col md:flex-row justify-between items-center md:items-start mb-4">
                            ...
                            <div class="flex gap-3 mt-4 md:mt-0">
                                <button 
                                    @click="openWhatsApp(selectedCandidate)"
