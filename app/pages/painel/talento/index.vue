...
import CandidaturasModal from '~/components/modals/CandidaturasModal.vue'
...
const fetchFullViews = async () => {
    if (!user.value) return
    loadingDetails.value = true
    // Using visualizacoes_vitrine as seen in server and migrations
    const { data } = await supabase
  ...
  <div class="min-h-screen bg-slate-50/50 pb-20">
    ...
    <div class="container mx-auto px-4 -mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <!-- Card Candidaturas -->
          <div @click="openCandidaturasModal" class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-green-100 transition-all cursor-pointer group">
            ...
            <p class="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">Candidaturas</p>
            <h3 class="text-3xl font-black text-slate-900">{{ stats.candidaturas }}</h3>
          </div>

          <!-- Card Visitas -->
          <div @click="openViewsModal" class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-100 transition-all cursor-pointer group">
            <div class="flex items-start justify-between mb-4">
    ...
    <!-- Modals -->
    <CandidaturasModal 
        v-model="showCandidaturasModal"
        :candidaturas="candidaturas"
        :loading="loadingCandidaturas"
    />
    <VisitasModal 
        v-model="showVisitasModal"
        :visitas="visitas"
    ...
    <div v-if="isCandidaturasModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div class="bg-white rounded-[32px] shadow-2xl w-full max-w-2xl relative overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
        ...
        <div class="flex-1 overflow-y-auto p-8 pt-4">
          ...
          <div v-else class="space-y-4">
            <div v-for="app in fullCandidaturas" :key="app.id" class="p-5 border border-slate-100 rounded-2xl hover:border-green-100 hover:bg-green-50/30 transition-all group">
    ...
    <div v-if="isViewsModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div class="bg-white rounded-[32px] shadow-2xl w-full max-w-2xl relative overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
        ...
        <div class="flex-1 overflow-y-auto p-8 pt-4">
          ...
          <div v-else class="space-y-4">
            <div v-for="view in fullViews" :key="view.id" class="p-5 border border-slate-100 rounded-2xl flex items-center justify-between group">
              <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-slate-100 rounded-xl overflow-hidden shadow-sm">
              ...
              <div class="text-right">
                ...
                <p class="text-xs font-bold text-slate-600">
                  {{ new Date(view.created_at).toLocaleDateString('pt-BR') }}
                </p>
              </div>
            </div>
          </div>
