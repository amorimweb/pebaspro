  ...
  <div class="bg-slate-50 min-h-screen">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <header class="text-center mb-16">
        <h1 class="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">Empresas da <span class="text-green-600">Região</span></h1>
        <p class="text-lg text-slate-500 font-medium">Conheça as empresas que estão contratando e fazendo a economia local girar.</p>
      </header>
      ...
      <!-- Grid de Empresas -->
      <div v-if="companies && companies.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <NuxtLink v-for="company in companies" :key="company.id" :to="`/empresas/${company.id}`" class="group bg-white p-8 rounded-[24px] shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-2 hover:border-green-500/30 transition-all duration-300 flex flex-col">
          ...
          <div class="flex-1">
            <div class="text-xs font-black text-green-600 uppercase tracking-widest mb-2">{{ company.profissao || 'Parceiro PebasPro' }}</div>
      ...
      <div v-else-if="!companies" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
