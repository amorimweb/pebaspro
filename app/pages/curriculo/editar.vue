...
const loadingMessages = [
    ...
    'Construindo seu objetivo profissional...',
    'Formatando suas experiências...',
    'Identificando competências-chave...',
    'Finalizando seu perfil de alto impacto...'
]
...
const submitAiAssistant = async () => {
    if (!aiQuestions.value.interesses || !aiQuestions.value.experiencia_resumida) {
        alert('Por favor, preencha pelo menos seus interesses e um resumo de sua experiência.')
        return
    }
    ...
    try {
        const payload = {
            ...
            userName: authStore.profile?.nome,
            questions: { ...aiQuestions.value }
        }

        // Webhook URL
        const WEBHOOK_URL = 'https://n8n-n8n-start.6uchwp.easypanel.host/webhook/curriculo-ai' 

        const response: any = await $fetch(WEBHOOK_URL, {
            method: 'POST',
            body: payload
        })

        console.log('AI Webhook Response:', response)

        if (response && response.message === 'Workflow was started') {
            alert('Atenção: O n8n está configurado para responder imediatamente. Mude a configuração do nó Webhook para "Respond: Using Respond to Webhook Node" para que os dados voltem para cá.')
        ...
        // Se o n8n retornar um campo "output" com string JSON (comum em IAs)
        if (data && data.output) {
            try {
                data = typeof data.output === 'string' ? JSON.parse(data.output) : data.output
            } catch (e) {
                console.error('Erro ao parsear campo output do n8n:', e)
            }
        }

        if (data) {
            // Preenchimento Automático com sanitização básica
            if (data.objetivo_profissional) form.value.objetivo_profissional = String(data.objetivo_profissional)
            if (data.biografia) form.value.biografia = String(data.biografia)
            
            if (data.habilidades) {
                const habs = typeof data.habilidades === 'string' ? data.habilidades.split(',').map(s => s.trim()) : data.habilidades
                form.value.habilidades = Array.isArray(habs) ? habs : []
            }
            
            if (data.experiencia_profissional) {
                try {
                   const exp = typeof data.experiencia_profissional === 'string' ? JSON.parse(data.experiencia_profissional) : data.experiencia_profissional
                   form.value.experiencia_profissional = Array.isArray(exp) ? exp : []
                } catch(e) { console.error('Erro ao processar experiências da IA:', e) }
            }

            if (data.formacao_academica) {
                try {
                   const edu = typeof data.formacao_academica === 'string' ? JSON.parse(data.formacao_academica) : data.formacao_academica
                   form.value.formacao_academica = Array.isArray(edu) ? edu : []
                } catch(e) { console.error('Erro ao processar formação da IA:', e) }
            }
            
            alert('IA: Campos preenchidos com sucesso! Revise as informações antes de salvar.')
            showAiModal.value = false
        } else {
            alert('IA: O webhook respondeu mas não retornou dados válidos.')
        }
    } catch (err) {
        console.error('Erro ao chamar assistente de IA:', err)
        alert('Ocorreu um erro ao processar sua solicitação com a IA. Verifique se o endereço do webhook está correto.')
    } finally {
        loadingAi.value = false
    }
}
...
const openPrintView = () => {
    // Navigate in same tab as requested
    navigateTo('/painel/talento/curriculo-print')
}
</script>
...
<template>
  <div class="min-h-screen bg-slate-50/50 pb-20">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div class="space-y-2">
                    ...
                    <h1 class="text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
                        Editar Meu Currículo ✍️
                    </h1>
                    <p class="text-slate-500 text-lg font-medium max-w-2xl">
                        Ajuste suas informações abaixo. Ao salvar, você será redirecionado para a visualização.
                    </p>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <NuxtLink 
                        to="/curriculo"
                        class="flex items-center gap-2 px-6 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition shadow-sm active:scale-95 text-lg"
                    >
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 mb-10">
            ...
            <div class="mt-8 pt-8 border-t border-slate-100">
                <div class="bg-blue-50/50 rounded-2xl p-6 border border-blue-100/50 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="font-black text-slate-900 uppercase tracking-tight">Assistente de Currículo IA</h4>
                            <p class="text-sm text-slate-500 font-medium">Responda 4 perguntas simples e deixe a IA preencher seu currículo.</p>
                        </div>
                    </div>
                    <button 
                        @click="showAiModal = true"
                        class="px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/10 active:scale-95 flex items-center gap-2 whitespace-nowrap"
                    >
                        Começar Agora
                        ...
                        <div v-if="isEditingSobre" class="grid grid-cols-1 gap-8">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex gap-3">
                                    <input v-model="newSkill" @keydown.enter.prevent="addSkill" type="text" class="flex-1 border-2 border-slate-100 bg-slate-50/50 rounded-2xl px-6 py-4 outline-none focus:border-green-500 focus:bg-white transition-all font-medium" placeholder="Digite e aperte Enter">
                            ...
                            <div class="pt-10 border-t border-slate-100">
                                ...
                                <div class="flex flex-col md:flex-row items-center gap-6">
                                    <button @click="getLocation" class="w-full md:w-auto px-8 py-5 bg-blue-50 text-blue-700 rounded-2xl font-black hover:bg-blue-100 transition flex items-center justify-center gap-3">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        ...
                        <div v-else class="grid grid-cols-1 gap-6">
                            <div v-for="(exp, index) in form.experiencia_profissional" :key="index" class="p-8 border-2 border-slate-50 bg-slate-50/30 rounded-3xl hover:border-green-200 hover:bg-white transition-all relative group shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                                ...
                                <div class="flex items-start gap-6">
                                    <div class="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center font-black text-2xl text-slate-200 uppercase">{{ exp.empresa?.charAt(0) }}</div>
                                    <div class="space-y-1">
        ...
        <div class="bg-white rounded-[40px] shadow-2xl p-10 w-full max-w-2xl relative animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2 space-y-2">
                    ...
                    <input v-model="tempExp.cargo" type="text" class="w-full border-2 border-slate-50 bg-slate-50/50 p-4 rounded-2xl outline-none focus:border-green-500 focus:bg-white transition-all font-bold">
                </div>
                <div class="md:col-span-2 space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Empresa</label>
                    <input v-model="tempExp.empresa" type="text" class="w-full border-2 border-slate-50 bg-slate-50/50 p-4 rounded-2xl outline-none focus:border-green-500 focus:bg-white transition-all font-bold">
                </div>
                <div class="space-y-2">
                ...
                <div class="md:col-span-2 py-2">
                    <label class="flex items-center gap-3 text-sm font-black text-slate-700 cursor-pointer">
                        <input v-model="tempExp.atual" type="checkbox" class="w-6 h-6 rounded-lg"> Trabalho aqui atualmente
                    </label>
                </div>
                <div class="md:col-span-2 space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Atividades</label>
                    <textarea v-model="tempExp.descricao" rows="4" class="w-full border-2 border-slate-50 bg-slate-50/50 p-4 rounded-2xl outline-none focus:border-green-500 focus:bg-white transition-all font-medium"></textarea>
                </div>
            </div>
            <div class="flex justify-end gap-3 mt-10 pt-6 border-t border-slate-100">
                <button @click="showExpModal = false" class="px-8 py-4 text-slate-400 font-black uppercase text-xs">Cancelar</button>
                <button @click="saveExp" class="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black">Salvar Experiência</button>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-[40px] shadow-2xl p-10 w-full max-w-2xl relative animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2 space-y-2">
                    ...
                    <input v-model="tempEdu.curso" type="text" class="w-full border-2 border-slate-50 bg-slate-50/50 p-4 rounded-2xl outline-none focus:border-green-500 focus:bg-white transition-all font-bold">
                </div>
                <div class="md:col-span-2 space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Instituição</label>
                    <input v-model="tempEdu.instituicao" type="text" class="w-full border-2 border-slate-50 bg-slate-50/50 p-4 rounded-2xl outline-none focus:border-green-500 focus:bg-white transition-all font-bold">
                </div>
                <div class="md:col-span-2 space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Nível</label>
                    <select v-model="tempEdu.nivel" class="w-full border-2 border-slate-50 bg-slate-50/50 p-4 rounded-2xl outline-none">
            ...
            <div class="flex justify-end gap-3 mt-10 pt-6 border-t border-slate-100">
                <button @click="showEduModal = false" class="px-8 py-4 text-slate-400 font-black uppercase text-xs">Cancelar</button>
                <button @click="saveEdu" class="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black">Salvar Formação</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showAiModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" @click="!loadingAi && (showAiModal = false)"></div>
        <div class="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
            <!-- Header -->
            <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-blue-600 text-white flex-shrink-0">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                    </div>
                    <div>
                        <h3 class="text-lg font-black uppercase tracking-widest">Assistente Mágico</h3>
                        <p class="text-[10px] font-bold text-blue-100 uppercase tracking-tight">Conte-nos sobre você</p>
                    </div>
                </div>
                <button @click="!loadingAi && (showAiModal = false)" :disabled="loadingAi" class="p-2 hover:bg-white/10 rounded-xl transition-colors disabled:opacity-30">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <!-- Content -->
            <div class="p-6 space-y-6 overflow-y-auto relative flex-1">
                <!-- Loading State overlay -->
                <div v-if="loadingAi" class="absolute inset-0 bg-white/95 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-10 text-center animate-in fade-in duration-300">
                    <div class="w-20 h-20 mb-8 relative">
                        <div class="absolute inset-0 border-4 border-blue-100 rounded-full"></div>
                        <div class="absolute inset-0 border-4 border-blue-600 rounded-full border-t-transparent animate-spin"></div>
                    </div>
                    <h4 class="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight">A Mágica está acontecendo...</h4>
                    <p class="text-blue-600 font-black text-sm mb-8 animate-pulse uppercase tracking-[0.2em]">{{ currentLoadingMessage }}</p>
                    
                    <!-- Progress Bar Simulation -->
                    <div class="w-full max-w-sm bg-slate-100 h-3 rounded-full overflow-hidden shadow-inner">
                        <div class="h-full bg-blue-600 animate-shimmer" style="width: 100%; background: linear-gradient(90deg, #2563eb 0%, #60a5fa 50%, #2563eb 100%); background-size: 200% 100%;"></div>
                    </div>
                    <p class="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mt-6 italic">Isso pode levar de 10 a 30 segundos</p>
                </div>

                <div :class="{ 'opacity-20 pointer-events-none blur-sm': loadingAi }" class="space-y-6 transition-all duration-500">
                    <div class="bg-blue-50 border border-blue-100 p-6 rounded-3xl flex items-start gap-4">
                        ...
                        <p class="text-sm text-blue-800 font-bold leading-relaxed">A IA usará estas respostas para criar um texto profissional, listar suas habilidades e organizar suas experiências automaticamente.</p>
                    </div>

                    <div class="space-y-6">
                        <div class="group">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block group-focus-within:text-blue-600 transition-colors">1. Qual seu objetivo ou área de interesse?</label>
                            <input v-model="aiQuestions.interesses" placeholder="Ex: Quero trabalhar como motorista de caminhão ou entregador" class="w-full bg-slate-50 border-2 border-transparent rounded-2xl px-6 py-5 focus:ring-0 focus:border-blue-400 focus:bg-white font-bold text-slate-700 transition-all placeholder:text-slate-300" />
                        </div>
                        <div class="group">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block group-focus-within:text-blue-600 transition-colors">2. Conte um pouco sobre sua experiência.</label>
                            <textarea v-model="aiQuestions.experiencia_resumida" rows="4" placeholder="Ex: Trabalhei 5 anos na transportadora X, viajava todo o Brasil com carga pesada e conheço bem mecânica básica" class="w-full bg-slate-50 border-2 border-transparent rounded-2xl px-6 py-5 focus:ring-0 focus:border-blue-400 focus:bg-white font-bold text-slate-700 transition-all placeholder:text-slate-300"></textarea>
                        </div>
                        <div class="group">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block group-focus-within:text-blue-600 transition-colors">3. Qual sua formação ou cursos?</label>
                            <input v-model="aiQuestions.formacao_resumida" placeholder="Ex: Ensino médio completo e curso de direção defensiva" class="w-full bg-slate-50 border-2 border-transparent rounded-2xl px-6 py-5 focus:ring-0 focus:border-blue-400 focus:bg-white font-bold text-slate-700 transition-all placeholder:text-slate-300" />
                        </div>
                        <div class="group">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block group-focus-within:text-blue-600 transition-colors">4. Quais são suas principais habilidades?</label>
                            <input v-model="aiQuestions.habilidades_principais" placeholder="Ex: Pontualidade, cuidado com a carga, CNH categoria E" class="w-full bg-slate-50 border-2 border-transparent rounded-2xl px-6 py-5 focus:ring-0 focus:border-blue-400 focus:bg-white font-bold text-slate-700 transition-all placeholder:text-slate-300" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Footer -->
            <div class="p-6 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-100 flex-shrink-0">
                <button @click="showAiModal = false" class="px-5 py-3 text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-slate-600 transition-all">Cancelar</button>
                <button 
                    @click="submitAiAssistant" 
                    :disabled="loadingAi"
                    class="px-10 py-4 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 active:scale-95 flex items-center gap-3 disabled:opacity-50"
                >
                    <svg v-if="loadingAi" class="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    {{ loadingAi ? 'A Mágica está acontecendo...' : 'Gerar Currículo com IA' }}
                </button>
            </div>
        </div>
    </div>
